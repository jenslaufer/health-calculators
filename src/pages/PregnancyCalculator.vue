...
useHead({
  title: "Pregnancy Due Date Calculator — Free Naegele's Rule Calculator",
  description: 'Calculate your pregnancy due date using Naegele\'s rule. Gestational age, trimester, conception date, progress bar, and milestone timeline. Free, instant, no sign-up.',
  path: '/pregnancy',
  jsonLd: {
...
const milestones = computed(() => {
  ...
  return [
    { week: 6, label: 'Heartbeat detectable', date: addDays(base, 6 * 7 + adj) },
    { week: 12, label: 'End of 1st trimester', date: addDays(base, 12 * 7 + adj) },
    { week: 16, label: 'Gender may be visible', date: addDays(base, 16 * 7 + adj) },
    { week: 20, label: 'Anatomy scan', date: addDays(base, 20 * 7 + adj) },
    { week: 24, label: 'Viability milestone', date: addDays(base, 24 * 7 + adj) },
    { week: 28, label: 'Start of 3rd trimester', date: addDays(base, 28 * 7 + adj) },
    { week: 37, label: 'Early term', date: addDays(base, 37 * 7 + adj) },
    { week: 40, label: 'Due date', date: dueDate.value },
  ]
})
...
<template>
  <div class="mb-10">
    ...
    <h1 class="text-4xl font-bold tracking-tight text-stone-900 mb-2">Pregnancy Due Date Calculator</h1>
    <p class="text-base text-stone-500 font-normal">Estimate your due date, gestational age, and pregnancy milestones.</p>
  </div>

  <div class="bg-white rounded-xl shadow-sm border border-stone-200 p-8 mb-6">
    <div class="grid gap-6 sm:grid-cols-2 mb-6">
      <div>
        <label for="lmp" class="block text-xs font-semibold text-stone-500 uppercase tracking-widest mb-2">Last Menstrual Period</label>
        <input
          id="lmp"
          v-model="lmp"
          type="date"
      ...
      <div>
        <label for="cycle" class="block text-xs font-semibold text-stone-500 uppercase tracking-widest mb-2">Cycle Length (days)</label>
        <input
          id="cycle"
    ...
    <div v-if="hasResults">
      <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 mb-6">
        <div class="rounded-xl border border-stone-200 p-5 text-center">
          <div class="text-xs font-semibold text-stone-400 uppercase tracking-widest mb-1">Due Date</div>
          <div class="text-xl font-bold text-stone-900" data-testid="due-date">{{ formatDate(dueDate) }}</div>
        </div>
        <div class="rounded-xl border border-stone-200 p-5 text-center">
          <div class="text-xs font-semibold text-stone-400 uppercase tracking-widest mb-1">Gestational Age</div>
          <div class="text-xl font-bold text-stone-900" data-testid="gestational-age">{{ gestationalWeeks }} weeks, {{ gestationalRemainderDays }} days</div>
        </div>
        <div class="rounded-xl border border-stone-200 p-5 text-center">
          <div class="text-xs font-semibold text-stone-400 uppercase tracking-widest mb-1">Trimester</div>
          <div class="text-xl font-bold text-stone-900" data-testid="trimester">{{ trimester }}</div>
        </div>
        <div class="rounded-xl border border-stone-200 p-5 text-center">
          <div class="text-xs font-semibold text-stone-400 uppercase tracking-widest mb-1">Conception Date</div>
      ...
      <div class="mb-6">
        ...
        <div class="w-full bg-stone-100 rounded-full h-3" data-testid="progress-bar">
          <div
            role="progressbar"
            class="bg-stone-700 h-3 rounded-full transition-all duration-300"
      ...
      <div>
        <h2 class="text-lg font-semibold text-stone-900 mb-4">Milestones</h2>
        <div class="space-y-3">
          <div
          ...
          >
            ...
            <span class="text-sm font-semibold text-stone-700 w-16">Week {{ m.week }}</span>
            <span class="text-sm text-stone-600 flex-1">{{ m.label }}</span>
  ...
  <div class="bg-white rounded-xl shadow-sm border border-stone-200 p-8 mb-6">
    ...
    <p class="text-sm text-stone-600 leading-relaxed">
      This calculator uses Naegele's rule: add 280 days to the first day of your last menstrual period (LMP).
      If your cycle is longer or shorter than 28 days, the due date is adjusted accordingly.
      Gestational age counts from LMP, not from conception.
    </p>
  </div>
