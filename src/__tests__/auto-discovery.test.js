...
describe('calculator discovery', () => {
  it('discovers all 21 calculators', () => {
    expect(calculatorMetas).toHaveLength(21)
  ...
  it('builds calculatorComponents map for all keys', () => {
    expect(Object.keys(calculatorComponents)).toHaveLength(21)
    for (const key of EXPECTED_KEYS) {
...
describe('blog component discovery', () => {
  it('discovers all 21 German blog components', () => {
    expect(Object.keys(blogComponentsDe)).toHaveLength(21)
    for (const slug of EXPECTED_BLOG_SLUGS_DE) {
      expect(blogComponentsDe[slug]).toBeDefined()
    }
  })

  it('discovers all 21 English blog components', () => {
    expect(Object.keys(blogComponentsEn)).toHaveLength(21)
...
describe('calculator groups', () => {
  ...
  it('groups contain all 21 calculators with no duplicates', () => {
    const allKeys = calculatorGroups.flatMap(g => g.calculators)
    expect(allKeys).toHaveLength(21)
    expect(new Set(allKeys).size).toBe(21)
    for (const key of EXPECTED_KEYS) {
  ...
  it('nutritionEnergy group has correct calculators in order', () => {
    expect(calculatorGroups[1].calculators).toEqual([
      ...
      'protein', 'caloriesBurned', 'intermittentFasting', 'keto',
    ])
  })

  it('fitnessRecovery group has correct calculators in order', () => {
    expect(calculatorGroups[2].calculators).toEqual([
      'heartRate', 'sleep', 'bloodPressure', 'vo2Max', 'oneRepMax', 'runningPace',
    ])
  })
...
describe('SSG routes', () => {
  it('generates exactly 132 routes', () => {
