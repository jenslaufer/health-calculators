...
describe('discoverMetas', () => {
  it('discovers all 67 calculator meta files', () => {
    const metas = discoverMetas(META_DIR)
    expect(metas).toHaveLength(67)
  })
...
describe('discoverBlogSlugs', () => {
  it('returns all 65 DE blog slugs', () => {
    const metas = discoverMetas(META_DIR)
    const { de } = discoverBlogSlugs(metas)
    expect(de).toHaveLength(65)
    for (const slug of EXPECTED_BLOG_SLUGS_DE) {
      expect(de, `missing de blog slug: ${slug}`).toContain(slug)
    }
  })

  it('returns all 65 EN blog slugs', () => {
    const metas = discoverMetas(META_DIR)
    const { en } = discoverBlogSlugs(metas)
    expect(en).toHaveLength(65)
...
describe('generateSitemap', () => {
  ...
  it('generates correct total URL count (2 home + 134 calcs + 2 blog index + 130 blog articles = 268)', () => {
    const urlCount = (xml.match(/<url>/g) || []).length
    expect(urlCount).toBe(268)
  })
