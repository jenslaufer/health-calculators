...
import { getEnRelatedArticles } from '../data/articles-en.js'

const props = defineProps({
  slug: { type: String, required: true },
})

const related = computed(() => getEnRelatedArticles(props.slug))
</script>

<template>
  <div v-if="related.length" class="mt-10" data-testid="related-articles">
    <h2 class="text-2xl font-bold text-stone-900 mb-4">Related Articles</h2>
    <div class="space-y-4">
      <router-link
        v-for="article in related"
        :key="article.slug"
        :to="`/en/blog/${article.slug}`"
      ...
      >
        ...
        <span class="text-sm font-medium text-stone-900 mt-2 inline-block">Read more &rarr;</span>
      </router-link>
    </div>
  </div>
</template>
